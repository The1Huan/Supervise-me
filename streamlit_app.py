...
def Supervise_me():
    ...
    if missing_columns:
        ...
        def show_teacher_theses(teacher):
            ...
            # Additional data for the teacher
            ...
            st.markdown(f"## Details for {teacher}")
            filtered_subject_data = subject_data[subject_data['Teacher'] == teacher]
            st.write(filtered_subject_data)
            filtered_expertise_data = expertise_data[expertise_data['Teacher'] == teacher]
            ...
            if st.button("Rechercher"):
                if user_input != "Tapez votre texte ici...":
                    ...
                    for index in similar_docs:
                        st.write(f"Titre: {data.iloc[index]['TitelInEnglisch']}")
                        ...
                        if st.button(f"Voir thèses de ce professeur ({data.iloc[index]['Teacher']})", key=f"teacher-{index}"):
...
def Statistics_of_teachers_demo():
    ...
    if missing_columns:
        ...
        if 'selected_teacher' in st.session_state:
            teacher_selection = st.session_state['selected_teacher']
...
# Handle navigation to the selected function
if 'navigate_to' in st.session_state:
    demo_name = st.session_state['navigate_to']
